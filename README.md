...
| 等级 | 含义 | 示例 | 最低动作 |
|---|---|---|---|
| L1 | 不改变事实的表达修订 | 语病、格式、非事实台词压缩 | 局部复验 |
| L2 | 局部剧情/连续性变化 | 道具易主、伤病时机、秘密提前 | 上游通知 + 连续性复验 |
| L3 | 结构或故事发动机变化 | 主角目标、中点、对手策略、高潮规则变化 | 完整影响传播 + 对抗式审查 |
| L4 | 项目级变化 | 长片改竖屏、横屏改 9:16、实拍改 AI、商业模式改变 | 重新立项级审查 |
