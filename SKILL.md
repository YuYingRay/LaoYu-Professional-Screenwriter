...
每次开始或恢复任务，先读当前 Manifest、它指向的用户反馈/制作经验和本次适用参考，再读实际拟用资产。
区分长期原则、当前决定、已替代方案和待验证假设；最新明确纠正优先，并同步到有效入口。
不能让旧模板、旧批准或局部修复把已否决的制作路线重新激活，也不让用户重复已有明确意见。

分镜图或视频 Prompt 任务（包括只交文字、由用户自行生成），先确定本次交付单位是单镜、整段还是整片，
再读 `references/ai-production-handoff.md` §1.4–1.5。用户要求继承过往制作方法时，带入已认可的图组职责、
Prompt 结构和交付方式；单镜模板、局部技术测试或新平台未知项不能自行缩小原委托。
按该参考的恢复与交付对照落实到实际产物；不能以“Skill 已写规则”判定经验迁移完成。

每次形成阶段成果、收到反馈或完成复盘，都提炼值得复用的成功做法与纠错经验，不等用户再次提醒。
通用方法进入对应 Skill 参考并从入口可达；项目专属选择留在项目权威记录且恢复时必读；
被替代意见保留历史但退出执行；尚无证据的原因保持假设。已有规则能覆盖且无新证据或边界时复用原处，
不为每次交付硬添条文。用户已授权持续维护时，完成适用 Skill 更新与复验，不能只记复盘或承诺下次注意。
具体闭环及维护授权边界见 `references/ai-production-handoff.md` §11.4。

生成分镜、图像或视频前，必须读取该参考 §3.3、§6.4–6.6，对照本镜状态检查实际参考像素与完整 Prompt。
关键矛盾未消解，不得通过手工工具调用绕过批次暂停；可继续不依赖该矛盾的已授权工作。
...
整集 AI 制作按该参考 §1.4–1.5 串联工作流；同场多图、原生多镜头 Prompt 与可操作交付分别见 §6.5、§7.2.1、§15.4。
AI 段落交接使用 `templates/production-handoff.md` §2.1 的逐段部分，或保留功能等价的已认可项目格式；
只提交资产清单、单镜样例或制作建议，不能算整段 Prompt 交付完成。
一套共用场域不等于所有时段共用一张图；角色正面参考也不自动证明背面服装正确。逐镜检查依据见 §6.6。
