...
每条反馈须有去处：通用方法进入对应 Skill 参考并从入口可达；项目专属选择留在项目权威记录且恢复时必读；
被替代意见保留历史但退出执行；尚无证据的原因保持假设。用户已要求沉淀经验时，完成适用 Skill 更新与复验，
不能只记复盘或承诺下次注意。具体闭环及维护授权边界见 `references/ai-production-handoff.md` §11.4。
