...
所有正式项目先建立 Full Bible。核心字段、连续性账和填写方法以
`references/story-bible-templates.md` 的 `# 5. 核心故事圣经模板`及
`templates/story-bible.md` 为准。

进入结构阶段前，至少能定位故事发动机、因果与信息边界、人物和关系状态、制作与权利
约束及未决风险；不适用项必须显式标记，不得静默省略。
...
每个 Scene Card 必须有稳定的 `SC-*`、明确的进入/退出状态，以及由人物行动造成的状态
变化。目标、阻碍、策略、转折、结果与因果接口的完整字段以
`templates/scene-card.md` 的 `# 3. 目标、阻碍、策略、转折、结果`和
`# 4. 因果位置`为准。
...
必须使用四个不可合并的独立角色；各角色立场、问题与风险清单以
`references/adversarial-review-rubric.md` 的 `# 3. 评审角色`为唯一主源。
