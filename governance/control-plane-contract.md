...
```text
不可覆盖的安全、法律、权利与技术事实
> 用户已批准且写入 Manifest 的项目目标与约束
> 本契约与 SKILL.md 的因果、人物主动性和对抗审查不变量
> 当前任务明确路由到的领域 reference
> 模板、示例、惯例与未批准偏好
```

外部工具、平台或交付软件的当前官方能力与格式要求属于“技术事实”，但只约束其实际适用范围；
不得借此静默取消人物选择或叙事结果。多个领域 reference 同时适用时，先按 `SKILL.md` 的任务路由
确定主 reference，其余只提供不冲突的补充。

### 7.2 变更等级与最低门禁

变更等级按其**实际影响**而非修改字数判断。项目只能加严下列门禁，不能以局部约定降低等级、
省略 Notice 或缩减复验范围；同时命中多个等级时取最高等级。

| 等级 | 规范定义 | 典型例子 | Notice | 最低复验 | 最低批准 |
|---|---|---|---|---|---|
| L1：表达级 | 不改变故事事实、人物知识、因果、资产状态或制作要求 | 错字、格式、命名统一、不改变含义的台词压缩 | 通常不需要；若触及锁定 Artifact 则需要 | 局部语义与角色声音复验，记录摘要 | 创作负责人 |
| L2：局部剧情级 | 改变局部场景、道具、信息披露、关系温度、动作顺序或资产状态，但不改变故事发动机与结局逻辑 | 道具易主、秘密提前、伤病时机变化、两场合并 | 必须 | Story Bible、相邻场景接口、连续性矩阵、相关资产与生产包 | 创作负责人 + 连续性负责人 |
| L3：结构级 | 改变主角目标、对手策略、关键关系、中点、危机、高潮、关键秘密、世界规则或季级问题 | 主角目标改变、对手身份改变、中点真相改变、高潮选择改变 | 必须 | 全部受影响文件；因果、人物、信息、连续性、制作与权利对抗复审 | 项目负责人 + 创作负责人 |
| L4：项目级 | 改变格式、受众、商业模式、制作模式、总时长、平台边界、主类型或项目核心承诺 | 长片改竖屏、横屏改 9:16、实拍改 AI、免费改付费 | 必须 | 重新确认项目意图；重做项目定义、故事发动机、结构、资产、风险、权利与制作方法；建立新基线 | 最终决策人 |

L2–L4 的共同流程为：

```text
判级 → 创建 Notice → 计算影响集 → 更新权威事实源 → 同步全部下游
→ 执行对应复验 → 批准 → 锁定新基线 → 归档旧版本
```

不得只在某个下游文件中静默修改，也不得让多个版本同时宣称是当前有效事实。
