...
verification_refs: [WP4B-FULL-VALIDATION, ROOT-CANDIDATE-VALIDATION, PHASE4-QUALITY-GATE-FAIL, E7-T2-R1-FAIL, E7-T2-R2-FAIL, E7-CONTENT-ROLLBACK-CONTROL-GATE]
...
- `MIG-E7-CONTENT-ROLLBACK`：`VERIFIED`。按用户裁决 B 对 13 个模型可见文件执行语义回退，
  ...
  与 strict 门禁通过；
- Notice 的控制迁移状态保持 `VERIFIED`，但原子激活质量门禁因模型可见加载集变化重新打开；
  回退后候选必须重跑 T1–T4 与 E.5，旧质量和 Token 结果不得复用；
...
new_digest: sha256:8a1ca85c7ff45cd841e6871c8b2bd0e7793bb6a9f584832aafda3eb8f4ba215f
disposition: CONTENT_CHANGED
reason: 将既有候选迁移、Phase 4 门禁、E.7 两轮失败及用户裁决 B 的内容层回退纳入 v0.2.2 影响闭包；active baseline 保持不变。
```
