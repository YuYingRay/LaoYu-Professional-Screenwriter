---
artifact_id: PROJECT-FEATURE-001
artifact_type: PROJECT_MANIFEST
project_id: PROJECT-FEATURE-001
project_baseline: PROJECT-FEATURE-v1.0.0
artifact_version: v1.0.0
status: LOCKED
owner: fixture.feature.owner
reviewer: fixture.feature.reviewer
approver: fixture.feature.approver
test_run_id: RUN-FEATURE-BASELINE
conformance_level: HUMAN_REVIEWED
lock_scope: FULL
content_digest: sha256:a3e3ba1d0636f75ccf3dad0e90fc08baa80e3ff10ed0fd67b7ae802c9be991c3
upstream_ids: []
project_stage: DEVELOPMENT
---

# 项目 Manifest

| 项目 | 内容 |
|---|---|
| 项目名 | 潮线之下 |
| 项目 ID | PROJECT-FEATURE-001 |
| 项目基线 | PROJECT-FEATURE-v1.0.0 |
| 格式 | 长片，约 105 分钟 |
| 受众 | 喜欢伦理悬疑与人物选择的成年观众 |
| 画幅 | 16:9 |
| 当前阶段 | DEVELOPMENT |
| 当前门禁 | PRODUCTION_READY（fixture 用于验证控制链） |

## 当前权威文件

- Story Bible：`development/story-bible.md`
- 结构大纲：`development/feature-outline.md`
- 剧本主源：`script/master/script.fountain`
- 审查报告：`development/review-reports/REVIEW-001.md`
- Production Handoff：`production/handoff/handoff.md`

## 锁定范围

- 主角目标、港口系统规则、SC-001 与 SC-002 的因果接口已锁定。
- 后续修改必须创建新版本并建立 Notice。
