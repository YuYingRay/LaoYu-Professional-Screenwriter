---
artifact_id: BIBLE-v1.0.0
artifact_type: STORY_BIBLE
project_id: PROJECT-VERTICAL-001
project_baseline: PROJECT-VERTICAL-v1.0.0
artifact_version: v1.0.0
status: LOCKED
owner: fixture.vertical.writer
reviewer: fixture.vertical.reviewer
approver: fixture.vertical.approver
review_id: REVIEW-001
test_run_id: RUN-VERTICAL-BASELINE
conformance_level: HUMAN_REVIEWED
lock_scope: FULL
content_digest: sha256:721eb79d0f177498474ce74226c23573d727426382388bf75be1a5493d77916d
upstream_ids: [PROJECT-VERTICAL-001]
---

# Full Bible

## 项目发动机

- 主角：林夏。
- 外部目标：在系统同步前拿到妹妹林澈的原始身份日志。
- 深层需求：停止替妹妹做决定，承认保护也可能是控制。
- 错误信念：只要自己承担所有风险，妹妹就不必知道真相。
- 对抗力量：周明与市民镜像系统。
- 对抗目标：删除异常身份节点，保住系统稳定性和个人职位。

## 世界规则

- 市民镜像每晚 02:00 同步身份状态。
- 旧门禁卡只能开启档案室一次，使用后留下访问日志。
- 离线原始日志不能被在线系统改写。

## 季级承诺

- 季级问题：林夏能否在保护妹妹的同时把决定权还给她？
- 核心钩子：林夏发现删除指令上有自己的旧签名。
- 解锁规则：付费点后的前 20 秒必须确认风险真实存在，并进入新行动。

## 关键连续性

- 林澈一直佩戴红色耳机，EP-001 到 EP-002 不得无解释消失。
- 林夏使用旧门禁卡后，周明可以追踪她的位置。

## 条件域

- 长片结构：NOT_APPLICABLE；项目为连续竖屏短剧。
- 16:9 制作：NOT_APPLICABLE；目标为 9:16。
- 复杂 AI 特效：OPEN；需在制作交接前决定。
